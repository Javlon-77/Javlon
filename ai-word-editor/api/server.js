...
app.post("/api/chat",async(req,res)=>{try{if(!client)return res.status(503).json({error:"GROQ_API_KEY Render Environment Variables ichida sozlanmagan."});const instruction=String(req.body?.instruction||"").trim();if(!instruction)return res.status(400).json({error:"So‘rov yuboring."});const documentText=String(req.body?.documentText||"");const system=`Siz professional AI Word Editor yordamchisisiz. Matnda [[IMAGE:...]] ko‘rinishidagi markerlar bo‘lsa ularni o‘zgartirmang, o‘chirmang va joyini foydalanuvchi so‘ramasa almashtirmang. Tahrirlash so‘ralsa changed=true va editedDocument to‘liq tahrirlangan matn bo‘lsin. Savol yoki maslahat bo‘lsa changed=false. Javob o‘zbek tilida, aniq va qisqa bo‘lsin. FAQAT JSON qaytaring: {"changed":true,"answer":"...","editedDocument":"..."}`;const response=await client.chat.completions.create({model,temperature:0.15,response_format:{type:"json_object"},messages:[{role:"system",content:system},{role:"user",content:`Hujjat:\n${documentText}\n\nSo‘rov:\n${instruction}`}]});let data;try{data=JSON.parse(cleanJson(response.choices?.[0]?.message?.content||""))}catch{return res.status(502).json({error:"AI javobini o‘qib bo‘lmadi."})}res.json({changed:Boolean(data.changed),answer:String(data.answer||"Javob tayyor."),editedDocument:String(data.editedDocument||""),documentId:String(req.body?.documentId||"")})}catch(e){console.error("CHAT ERROR",e);res.status(Number(e?.status)||500).json({error:e?.error?.message||e?.message||"AI bilan bog‘lanishda xatolik."})}});
...
function imageSize(mime){const sizes={"image/png":[700,480],"image/jpeg":[700,480],"image/jpg":[700,480],"image/webp":[700,480],"image/gif":[500,400]};return sizes[mime]||[650,450]}
async function buildDocxWithBlocks(text,appendBlocks,meta){
  const original=fs.existsSync(filePath(meta.id))?fs.readFileSync(filePath(meta.id)):null;
  const hasImages=Array.isArray(appendBlocks)&&appendBlocks.some(b=>b?.type==="image"&&b.dataUrl);
  if(!hasImages&&original){const zip=await JSZip.loadAsync(original);const f=zip.file("word/document.xml");if(!f)throw new Error("DOCX document.xml topilmadi.");const xml=await f.async("string");zip.file("word/document.xml",applyText(xml,text));return zip.generateAsync({type:"nodebuffer",compression:"DEFLATE"})}
  const children=[];
  for(const line of String(text||"").split(/\r?\n/))children.push(new Paragraph({children:[new TextRun(line)]}));
  for(const block of Array.isArray(appendBlocks)?appendBlocks:[]){
    if(block?.type==="image"&&block.dataUrl){const {mime,buffer}=dataUrlToBuffer(block.dataUrl);const [width,height]=imageSize(mime);children.push(new Paragraph({children:[new ImageRun({data:buffer,type:mime.split("/")[1]==="jpg"?"jpeg":mime.split("/")[1],transformation:{width,height}})]}));if(block.caption)children.push(new Paragraph({children:[new TextRun(String(block.caption))]}));}
  }
  const doc=new Document({sections:[{children:children.length?children:[new Paragraph({children:[new TextRun("")]})]}]});
  return Packer.toBuffer(doc);
}

app.post("/api/save",async(req,res)=>{try{const id=String(req.body?.documentId||"");if(!id)return res.status(400).json({error:"Avval Word faylini oching."});const meta=getMeta(id);if(!meta)return res.status(404).json({error:"Word fayl topilmadi."});const text=String(req.body?.text||"");const appendBlocks=Array.isArray(req.body?.appendBlocks)?req.body.appendBlocks:[];const out=await buildDocxWithBlocks(text,appendBlocks,meta);fs.writeFileSync(filePath(id),out);const updatedAt=new Date().toISOString();upsertMeta({...meta,updatedAt});res.json({ok:true,documentId:id,fileName:meta.name,updatedAt,size:out.length})}catch(e){console.error("SAVE ERROR",e);res.status(500).json({error:e?.message||"Word faylni saqlab bo‘lmadi."})}});
app.post("/api/export",async(req,res)=>{try{const id=String(req.body?.documentId||"");const meta=getMeta(id);if(!meta)return res.status(404).json({error:"Word fayl topilmadi."});if(typeof req.body?.text==="string")await (async()=>{const blocks=Array.isArray(req.body?.appendBlocks)?req.body.appendBlocks:[];const out=await buildDocxWithBlocks(req.body.text,blocks,meta);fs.writeFileSync(filePath(id),out)})();res.setHeader("Content-Type","application/vnd.openxmlformats-officedocument.wordprocessingml.document");res.setHeader("Content-Disposition",`attachment; filename="${meta.name.replace(/\"/g,"")}"`);res.send(fs.readFileSync(filePath(id)))}catch(e){console.error("EXPORT ERROR",e);res.status(500).json({error:e?.message||"Word fayl tayyorlanmadi."})}});
